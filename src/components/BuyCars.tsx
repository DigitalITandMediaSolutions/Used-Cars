...
export default function BuyCars({
  ...
  const quickCategories = [
    { label: 'All Pre-owned', action: () => handleResetFilters(), isActive: !bodyType && !isCertified && !fuelType },
    { label: 'Certified Platinum', action: () => { handleResetFilters(); setIsCertified(true); setFilters({ isCertified: true }); }, isActive: isCertified },
    { label: 'SUVs & Crossovers', action: () => { handleResetFilters(); setBodyType('SUV'); setFilters({ bodyType: 'SUV' }); }, isActive: bodyType === 'SUV' },
    { label: 'Electric Vehicles', action: () => { handleResetFilters(); setFuelType('Electric'); setFilters({ fuelType: 'Electric' }); }, isActive: fuelType === 'Electric' },
    { label: 'Budget Hatchbacks', action: () => { handleResetFilters(); setBodyType('Hatchback'); setFilters({ bodyType: 'Hatchback' }); }, isActive: bodyType === 'Hatchback' }
  ];
              ...
              {sortedAndFilteredCars.map(car => {
                ...
                return (
                  <motion.div 
                    ...
                    className="bg-white border border-gray-150/80 rounded-2xl shadow-sm hover:shadow-lg transition-all overflow-hidden flex flex-col group relative"
                  >
      ...
      <AnimatePresence>
        {selectedCarDetail && (() => {
          ...
          const alternatesList = [
            ...
            { title: "Certified Engine", url: "https://images.unsplash.com/photo-1486006920555-c77dce18193b?auto=format&fit=crop&w=800&q=80" },
          ...
          const activeImageIndex = is360Mode 
            ? Math.min(4, Math.floor(angle360 / 72.5)) 
            : selectedImgIdx;
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="relative h-80 rounded-2xl overflow-hidden bg-slate-100 border border-gray-150 shadow-inner group">
                        ...
                        {!is360Mode && (
                          <>
                            <button 
                              onClick={() => setSelectedImgIdx(prev => (prev === 0 ? 4 : prev - 1))}
                            ...
                            <button 
                              onClick={() => setSelectedImgIdx(prev => (prev === 4 ? 0 : prev + 1))}
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          onClick={() => setContactAlert(`Dialing +1 (800) VELOCITY direct branch desk. Connecting you to Priya Sharma regarding the ${selectedCarDetail.year} ${selectedCarDetail.brand} ${selectedCarDetail.model} shortly.`)}
